...
const SignUp = () => {
  ...
  const validateForm = () => {
    if (!form.name) {
      Alert.alert("Error", t("nameRequired")); // Use translation key
      return false;
    }
    if (!form.phone) {
      Alert.alert("Error", t("phoneRequired")); // Use translation key
      return false;
    }
    if (!/^\d{10}$/.test(form.phone)) {
      Alert.alert("Error", t("phoneInvalid")); // Use translation key
      return false;
    }
    if (!form.state) {
      Alert.alert("Error", t("stateRequired")); // Use translation key
      return false;
    }
    if (!form.district) {
      Alert.alert("Error", t("districtRequired")); // Use translation key
      return false;
    }
    return true;
  };
  ...
  const onSignUpPress = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        Alert.alert("Error", errorData.error || t("mobileAlreadyRegistered")); // Use translation key
      }
    } catch (err) {
      Alert.alert("Error", t("errorOccurred")); // Use translation key
    } finally {
      setLoading(false);
    }
  };
